...
import FabricWeightFields from "../components/FabricWeightFields.jsx";
          ...
          <tbody>
            {filtered.map((i) => (
              <tr key={i.id} className="border-b border-line last:border-0 hover:bg-paper">
                ...
                <td className="px-4 py-3 font-semibold">{i.rfdRate ? `₹${formatCurrency(i.rfdRate)}` : "—"}</td>
                <td className="px-4 py-3 font-semibold">{i.dyedRate ? `₹${formatCurrency(i.dyedRate)}` : "—"}</td>
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(form); }} className="grid sm:grid-cols-2 gap-3">
      ...
      <FabricWeightFields
        ...
        onChangeAll={(result) => setForm((f) => ({
          ...f,
          gsm: result.gsm !== null ? String(result.gsm) : f.gsm,
          glm: result.glm !== null ? String(result.glm) : f.glm,
          oz: result.oz !== null ? String(result.oz) : f.oz,
        }))}
      />
